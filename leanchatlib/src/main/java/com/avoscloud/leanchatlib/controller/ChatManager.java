...
public class ChatManager extends AVIMClientEventHandler {
     ...
     * @param callback
     */
    public void fetchConversationWithUserId(final String userId, final AVIMConversationCreatedCallback callback, final int count) {
        AVIMConversationQuery query = imClient.getQuery();
        query.withMembers(Arrays.asList(userId, selfId));
        query.whereEqualTo(ConversationType.ATTR_TYPE_KEY,count);
            ...
            public void done(List<AVIMConversation> conversations, AVIMException e) {
                if (e != null) {
                    ...
                    if (conversations.size() > 0) {
                        ...
                        attrs.put(ConversationType.TYPE_KEY,count);//ConversationType.Single.getValue()
    ...
    List<AVIMTypedMessage> filterTypedMessages(List<AVIMMessage> messages) {
        List<AVIMTypedMessage> resultMessages = new ArrayList<AVIMTypedMessage>();
    ...
    public synchronized AVIMTypedMessage queryLatestMessage(AVIMConversation conversation) throws InterruptedException {
        ...
        final List<AVIMTypedMessage> typeMessages = new ArrayList<AVIMTypedMessage>();
