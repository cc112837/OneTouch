...
/**
 * Created by wli on 15/9/30. 自定义的 AVUser
 */
public class LeanchatUser extends AVUser {
	...
	public static void signUpByNameAndPwd(String name, String password,
	...
	public static void signUpByNameAndPwdAndProperty(String name, String password, String property,
			SignUpCallback callback) {
		AVUser user = new AVUser();
		user.setUsername(name);
		user.setPassword(password);
		user.put("property", property);
		user.signUpInBackground(callback);
	}
	...
	public void findFriendsWithCachePolicy(AVQuery.CachePolicy cachePolicy,
			FindCallback<LeanchatUser> findCallback) {
