...
import com.wzy.mhealth.LeanChat.viewholder.ContactItemHolder;
...
public class ContactsAdapter extends
		HeaderListAdapter<ContactsAdapter.ContactItem> {
	...
	/**
	 * 设置成员列表，然后更新索引 此处会对数据以 空格、数字、字母（汉字转化为拼音后的字母） 的顺序进行重新排列
	 */
...
//				 PinyinFormat.WITHOUT_TONE);
		...
		List<ContactItem> contactList2 = new ArrayList<ContactItem>();
		if (contactList != null && contactList.size() > 0) {
			contactList2.add(contactList.get(0));
			contactList2.addAll(contactList);
			super.setDataList(contactList2);
		} else
